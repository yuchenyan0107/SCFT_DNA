...
def _even_extension_2d(A, nx, ny):
    A2 = xp.empty((nx * 2, ny * 2), dtype=DTYPE)
    A2[:nx, :ny] = A
...
def solve_qsr_2d_fft_even_expend(Scft_params, wsr, qsr_initial, reverse):

    qsr = xp.zeros((Scft_params.ns+1, Scft_params.nx, Scft_params.ny), dtype=DTYPE)
    ...
    #ky_r = xp.fft.rfftfreq(ny2, d=Scft_params.dy)
    #K2 = (2.0 * xp.pi * kx)[:, None] ** 2 + (2.0 * xp.pi * ky_r)[None, :] ** 2
    ...
    q_real_extend = xp.empty((nx2, ny2), dtype=DTYPE)  # extended
...
def solve_qsr_2d_dct_expand(Scft_params, wsr, qsr_initial, reverse):
    ...
    qsr = xp.zeros((Scft_params.ns + 1, nx, ny), dtype=DTYPE)
